...
data_dir = '../ntust-ir-2020_hw5_new'
OUT_PATH = '../result.csv'
...
def _compute_score(args):
    ...
    i_idx_list = np.array([i for i, term in enumerate(dic) if term in intersection_q_doc], dtype=np.int32)
...
tf_doc = np.array(compute_tf(dic, doc_list))
tf_query = np.array(compute_tf(dic, query_list))
n_list = np.array(compute_n(dic, doc_list))
idf_list = np.array(compute_idf(N, n_list))
